...
void VirtualSensor::matchParams()
{
    for (auto& param : paramMap)
    {
        auto& name = param.first;
        auto& data = param.second;
        if (symbols.get_variable(name))
        {
            std::string objpath = data->getParamPath();

            /* Setup match for interfaces removed */
            auto handleDbusSignalNameOwnerChanged =
                [this, objpath](sdbusplus::message::message& message) {
                std::string name;
                std::string oldOwner;
                std::string newOwner;
                ...
                if (!oldOwner.empty() && !name.empty())
                {
                    if (!name.starts_with(":"))
                    {
                        // Connection removed
                        double value = std::numeric_limits<double>::quiet_NaN();
                        updateVirtualSensorBySignal(
                            name, value, SignalType::nameOwnerChange);
                    }
                }
            };

            auto matchNameOwnerChanged =
                std::make_unique<sdbusplus::bus::match::match>(
                    bus, sdbusplus::bus::match::rules::nameOwnerChanged(),
                    handleDbusSignalNameOwnerChanged);

            this->matches.emplace_back(std::move(matchNameOwnerChanged));
        }
        else
        {
            /* Invalid parameter */
            throw std::invalid_argument(
                "matchParams:ParamName not found in symbols");
        }
    }
    return;
}
                                                ...
                                                SignalType signalType)
{
    for (auto& param : paramMap)
    {
        ...
        if (auto var = symbols.get_variable(name))
        {
            ...
            if (str == data->getParamPath())
            {
                // interfaceRemoved can get sensor Path
                if (signalType == SignalType::interfaceRemoved)
                {
                    data->clearSensorValue();
                }
