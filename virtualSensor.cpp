...
double VirtualSensor::calculateValue(std::string calc,
                                     const VirtualSensor::ParamMap& paramMap)
{
    auto itr =
        std::find(calculationTypes.begin(), calculationTypes.end(), calc);
    if (itr == calculationTypes.end())
    ...
    for (auto& param : paramMap)
    {
        ...
        if (auto var = symbols.get_variable(name))
        {
            if (std::isnan(var->ref()))
                continue;
            values.push_back(std::clamp(var->ref(), ValueIface::minValue(),
                                        ValueIface::maxValue()));
        }
    }
...
{
    // do we want to check and/or enforce the lower threshold being lower than
    // the upper?
