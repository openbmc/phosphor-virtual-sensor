...
double VirtualSensor::calculateValue(std::string calculation,
                                     const VirtualSensor::ParamMap& paramMap)
{
    auto itr = std::find(calculationTypes.begin(), calculationTypes.end(), calculation);
    if (itr == calculationTypes.end())
    {
...
std::string VirtualSensor::getThresholdType(std::string name, int severity)
{
...
void VirtualSensors::createVirtualSensorsFromDBus(const std::string& calculationType)
{
...
int main()
{
    ...
    for (const char* type : calculationTypes)
    {
        auto match = std::make_unique<sdbusplus::bus::match::match>(
            bus, std::string("type='signal',member='PropertiesChanged',"
            "path_namespace='/xyz/openbmc_project/inventory',"
            "arg0namespace='") + vsConfigIntfPrefix + type + "'",
            eventHandler);
