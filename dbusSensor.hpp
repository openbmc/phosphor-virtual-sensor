...
class DbusSensor
{
  public:
     ...
     */
    DbusSensor(sdbusplus::bus::bus& bus, std::string path) :
        bus(bus), path(path)
    {
        servName = getService(bus, path.c_str(), sensorIntf);
    }

    /** @brief Get sensor value property from D-bus interface */
    double getSensorValue()
    {
        auto propValue = getDbusProperty(bus, servName.c_str(), path.c_str(),
                                         sensorIntf, "Value");
