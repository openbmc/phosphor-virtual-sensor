...
{
    if (service.empty() || path.empty() || intf.empty() || property.empty())
    {
        lg2::info(
            "Error set dbus property service:{SERVICE} path:{PATH} intf:{INTF} Property:{PROP}",
            "SERVICE", service, "PATH", path, "INTF", intf, "PROP", property);
        return -1;
    }

    auto method =
        bus.new_method_call(service.c_str(), path.c_str(), propIntf, methodSet);
    ...
    {
        lg2::error(
            "Error set dbus property service:{SERVICE} path:{PATH} intf:{INTF} Property:{PROP},{ERRMSG}",
            "SERVICE", service, "PATH", path, "INTF", intf, "PROP", property,
            "ERRMSG", e.what());
        return -1;
    }
